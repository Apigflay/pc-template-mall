    ...
    <div class="body_main">
        ...
        <div class="main" :style="widthMain">
            ...
            <div class="tips" id="" style="background:#cecece;text-align:left">
                <Tag v-for="(item,index) in tagData" :key="index" type="dot" closable color="primary" >{{item.index[2].name}}</Tag>
            </div>
            <div class="tips" id="" style="background:#cecece;text-align:left">
                <Tag v-for="(item,index) in count" :key="index" :name="item" type="dot" :color="item==colorid?'primary':'default'" checkable closable @on-close="handleClose2" @on-change="goChange">标签{{ item + 1 }}</Tag>
            </div>
          <!-- 查询 新增 -->
          <div class="top">
            <label>支付类型：</label>
              <Select v-model="query" size="small" style="width:150px">
                  <Option v-for="item in pay_mode" :value="item.name" :key="item.id">{{ item.name }}</Option>
              </Select>
          ...
          <!-- 新增页面 -->
            <div class="pop" v-if="showNonecreate">
                <Form :label-width="80">
                    <FormItem label="支付类型：">
                        <Select v-model="pay">
                            <Option v-for="(item,index) in pay_mode"
                            :key="index"
                            :label="item.name"
                            :value="item.id">
                            </Option>
...
export default {
  ...
  created() { // 网页table表格缩小自动适应
    window.addEventListener('resize', this.getWidth);
    this.getWidth();
    this.getTag();
  },
  destroyed () { // 网页table表格缩小自动适应
  ...
  methods: {
      ...
      goChange:function(checked, name){
          if(checked){
            this.color='primary';
          }else{
            this.color='default';
